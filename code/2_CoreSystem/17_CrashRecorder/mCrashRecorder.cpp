...
#include "mCrashRecorder.h"

#ifdef USE_MODULE_CORE__CRASH_RECORDER

int8_t mCrashRecorder::Tasker(uint8_t function, JsonParserObject obj)
{
...
void mCrashRecorder::Init(void)
{
  
}

void mCrashRecorder::BootMessage()
{
  #ifdef ENABLE_FEATURE_SYSTEM__SHOW_BOOT_MESSAGE
  char buffer[100];
  if(bmp_count)
  {
    mCrashRecorder::appendToBuffer(buffer, sizeof(buffer), "#%d ", bmp_count);  
    char buffer2[50];
    for(uint8_t sensor_id = 0; sensor_id<bmp_count; sensor_id++)
    {      
      mCrashRecorder::appendToBuffer(buffer, sizeof(buffer), "%s:\"%s\", ", bmp_sensors[sensor_id].bmp_name, DLI->GetDeviceName_WithModuleUniqueID( GetModuleUniqueID(), sensor_id, buffer2, sizeof(buffer2)));    
    }
  }
  else{
    mCrashRecorder::appendToBuffer(buffer, sizeof(buffer), "None");  
  }
  mCrashRecorder::removeTrailingComma(buffer);
...
// Clear the RTC dump counter when we do a normal reboot, this avoids garbage data to stay in RTC
void mCrashRecorder::CrashDumpClear(void)
{
...
void mCrashRecorder::CrashDump(void)
{
...
// // void mCrashRecorder::DebugFreeMem(void)
// // {
...
// // Generate a crash to test the crash recorder
// void mCrashRecorder::CmndCrash(void)
// {
//   volatile uint32_t dummy;
//   dummy = *((uint32_t*) 0x00000000);
// }
...
// // Do an infinite loop to trigger WDT watchdog
// void mCrashRecorder::CmndWDT(void)
// {
//   volatile uint32_t dummy = 0;
//   while (1) {
//     dummy++;
...
// // This will trigger the os watch after OSWATCH_RESET_TIME (=120) seconds
// void mCrashRecorder::CmndBlockedLoop(void)
// {
...
// bool mCrashRecorder::CrashFlag(void)
// {
...
// void mCrashRecorder::CrashDump_AddJson(void)
// {
...
uint8_t mCrashRecorder::ConstructJSON_Settings(uint8_t json_level, bool json_appending){

  char buffer[50];
  JBI->Start();
    JBI->Add_P(PM_SENSOR_COUNT, 0);

  return JBI->End();

}

uint8_t mCrashRecorder::ConstructJSON_Sensor(uint8_t json_level, bool json_appending){
...
void mCrashRecorder::MQTTHandler_Init()
{

  struct handler<mCrashRecorder>* ptr;
  ...
  ptr->ConstructJSON_function = &mCrashRecorder::ConstructJSON_Settings;
  ...
  ptr->ConstructJSON_function = &mCrashRecorder::ConstructJSON_Sensor;
  mqtthandler_list.push_back(ptr);
