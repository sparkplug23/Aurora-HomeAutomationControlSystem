...
static esp_err_t capture_handler(httpd_req_t *req)
{
  ...
  Serial.println("capture_handler Start");
...
static esp_err_t stream_handler(httpd_req_t *req)
{
  ...
  Serial.println("stream_handler start");
  ...
  while(true){
    // Serial.println("stream_handler true");
      detected = false;
      face_id = 0;
      fb = esp_camera_fb_get();
      if (!fb) {
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
  ...
  Serial.println("cmd_handler start");
  ...
  if (buf_len > 1) {
      ...
      if(!buf){
          httpd_resp_send_500(req);
          Serial.println("httpd_resp_send_500");
          return ESP_FAIL;
      }
      if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
          if (httpd_query_key_value(buf, "var", variable, sizeof(variable)) == ESP_OK &&
              httpd_query_key_value(buf, "val", value, sizeof(value)) == ESP_OK) {
          Serial.println("httpd_query_key_value");
          } else {
          Serial.println("httpd_query_key_value else");
              free(buf);
              httpd_resp_send_404(req);
              return ESP_FAIL;
          }
      } else {
          Serial.println("httpd_resp_send_404 a");
          free(buf);
          httpd_resp_send_404(req);
          return ESP_FAIL;
      }
      free(buf);
          ...
          Serial.println("httpd_resp_send_404 b");
      httpd_resp_send_404(req);
      return ESP_FAIL;
  }

          Serial.println("int val = atoi(value); a");
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK) {
      ...
      {
        Serial.println("Stream URI Enabled Y");
      }else{
        Serial.println("Stream URI Enabled N");
      }
      Serial.println("Stream URI Enabled");
    }else{
      Serial.println("Stream URI ERROR");
    }
}
...
void mWebCamera::Init()
{
  ...
  if (err != ESP_OK) {
    ...
    ALOG_INF(PSTR(D_LOG_CAMERA "Init Success"));
  }
...
#endif // USE_MODULE_DRIVERS_CAMERA_ARDUINO
