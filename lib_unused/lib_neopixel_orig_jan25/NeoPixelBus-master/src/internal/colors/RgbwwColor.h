...
// #define ENABLE_RGBCCT_RGBWW_MIGRATE
    #ifdef ENABLE_RGBCCT_RGBWW_MIGRATE

// Set RGB values without modifying white channels
...
// Comparison methods
inline bool operator==(const RgbwwColor& other) const {
    return (R == other.R && G == other.G && B == other.B && WW == other.WW && CW == other.CW);
}
...
inline void setBrightness(uint8_t bri) {
    WW = CW = R = G = B = (bri * 255) >> 8;
}

inline uint8_t getBrightness() const {
    return std::max({R, G, B, WW, CW});
}

// Blend functions
static inline RgbwwColor LinearBlend(const RgbwwColor& left, const RgbwwColor& right, float progress) {
    return RgbwwColor(
        ...
        static_cast<uint8_t>(left.WW + (right.WW - left.WW) * progress),
        static_cast<uint8_t>(left.CW + (right.CW - left.CW) * progress)
    );
}

inline void setHue360(uint16_t hue) {
    HsbColor hsb = HsbColor(RgbColor(R, G, B));
    hsb.H = hue / 360.0f;
    *this = RgbwwColor(hsb);
}

inline void setSat255(uint8_t sat) {
    HsbColor hsb = HsbColor(RgbColor(R, G, B));
    hsb.S = sat / 255.0f;
    *this = RgbwwColor(hsb);
}

inline uint16_t getHue360() const {
    return static_cast<uint16_t>(HsbColor(RgbColor(R, G, B)).H * 360.0f);
}

inline uint8_t getSat255() const {
    return static_cast<uint8_t>(HsbColor(RgbColor(R, G, B)).S * 255.0f);
}

inline RgbwwColor WithBrightness(uint8_t bri) const {
    return RgbwwColor(
        (R * bri) >> 8,
        (G * bri) >> 8,
        (B * bri) >> 8,
...
// Debug print
inline void debug_print(const char* name = nullptr) const {
    if (name) {
        Serial.print(name);
        Serial.print("=");
    }
...
#endif // ENABLE_RGBCCT_RGBWW_MIGRATE
